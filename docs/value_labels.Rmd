...
that generalizes the concepts introduced by SAS, Stata, SPSS, et al., is type
safe, and can be used with tidyverse wrangling functions in powerfully
expressive ways. Luckily there was a lot of existing work in this area I was able
to build on!

NOTE THAT THIS IS VERY FAR FROM BEING PRODUCTION READY! There's a lot to still
...
This behavior was inspired by the `haven` package in the `tidyverse` universe.
...
the `haven_labelled` class to also support factors as a base type (`fct+lbl`).
